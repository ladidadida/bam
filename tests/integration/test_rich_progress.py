...
def test_rich_progress_in_interactive_mode_completes() -> None:
    ...
    result = subprocess.run(
        ["uv", "run", "bam", "--config", str(config_path), "package", "--jobs", "4"],
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def test_rich_progress_disabled_in_non_tty() -> None:
    ...
    result = subprocess.run(
        ["uv", "run", "bam", "--config", str(config_path), "package", "--jobs", "4", "--no-cache"],
...
""")
    ...
    result = subprocess.run(
        ["uv", "run", "bam", "--config", str(config), "task2", "--jobs", "1", "--no-cache"],
        capture_output=True,
        text=True,
        cwd=Path(__file__).parent.parent.parent,
        timeout=30,
