...
from cascade.executor import TaskExecutor
from cascade.graph import build_task_graph, execution_order_for_targets
...
def test_cli_graph_ascii(multi_stage_workspace: Path, runner: CliRunner):
    """Test 'cascade graph' ASCII output."""
    result = runner.invoke(
        app,
        [
            "graph",
            "--config",
            str(multi_stage_workspace / "cascade.yaml"),
            "--format",
            "ascii",
        ],
    )
